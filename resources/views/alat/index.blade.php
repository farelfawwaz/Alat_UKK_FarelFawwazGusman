                        ...
                        <tr class="hover:bg-blue-50 transition">
                            ...
                            <td class="px-6 py-4 text-center">
                                @if ($alat->status === 'tersedia')
                                    <span
                                        class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold bg-green-100 text-green-800 border border-green-300">
                                        <span class="w-2 h-2 bg-green-600 rounded-full"></span>
                                        Tersedia
                                    </span>
                                @else
                                    <span
                                        class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold bg-yellow-100 text-yellow-800 border border-yellow-300">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-center gap-2">
                                    ...
                                    <a href="{{ route('alat.edit', $alat->id) }}"
                                        class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-400 to-orange-400 text-white rounded-lg shadow hover:shadow-lg transition">
                                    ...
                                    <form action="{{ route('alat.destroy', $alat->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button onclick="return confirm('Yakin ingin menghapus alat ini?')"
                                            class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-red-500 to-rose-500 text-white rounded-lg shadow hover:shadow-lg transition">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                viewBox="0 0 24 24">
