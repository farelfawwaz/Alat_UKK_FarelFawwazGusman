...
@section('content')
    <div class="max-w-6xl mx-auto">
                    ...
                    <tr class="hover:bg-blue-50 transition">
                                ...
                                <div class="w-14 h-14 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                                    @if ($item->alat->image)
                                        <img src="{{ asset('storage/' . $item->alat->image) }}"
                                            alt="{{ $item->alat->nama_alat }}" class="w-full h-full object-cover">
                                    @else
                        ...
                        <td class="px-6 py-4 text-center">
                            @if ($item->status === 'menunggu')
                                <span
                                    class="px-3 py-1 text-xs font-semibold bg-yellow-100 text-yellow-700 rounded-full inline-block">
                                    Menunggu
                                </span>
                            @elseif ($item->status === 'disetujui')
                                <span
                                    class="px-3 py-1 text-xs font-semibold bg-blue-100 text-blue-700 rounded-full inline-block">
                                    Disetujui
                                </span>
                            @elseif ($item->status === 'dikembalikan')
                                <span
                                    class="px-3 py-1 text-xs font-semibold bg-green-100 text-green-700 rounded-full inline-block">
                                    Dikembalikan
                                </span>
                            @elseif ($item->status === 'ditolak')
                                <span
                                    class="px-3 py-1 text-xs font-semibold bg-red-100 text-red-700 rounded-full inline-block">
                                    Ditolak
                                </span>
                            @endif
                        </td>
        ...
        <div id="rejectionModal"
            class="fixed inset-0 bg-black/40 backdrop-blur-sm hidden items-center justify-center z-50 transition duration-300">

            <div class="bg-white rounded-xl shadow-lg w-full max-w-md p-6">
                <h2 class="text-lg font-bold mb-2 text-red-600">Alasan Penolakan</h2>

                <p class="text-sm text-gray-500 mb-1">Nama Alat:</p>
                <p id="modalNamaAlat" class="font-semibold mb-3"></p>

                <p class="text-sm text-gray-500 mb-1">Alasan:</p>
                <p id="modalAlasan" class="mb-4"></p>

                <div class="text-right">
                    <button onclick="closeRejectionModal()" class="px-4 py-2 bg-gray-200 rounded-lg hover:bg-gray-300">
                        Tutup
                    </button>
                </div>
            </div>
    ...
    <script>
        function openRejectionModal(namaAlat, alasan) {
